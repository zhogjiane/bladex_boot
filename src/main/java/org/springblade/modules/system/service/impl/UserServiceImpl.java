...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements IUserService {
	...
	public boolean submit(User user) {
		...
		Long cnt = baseMapper.selectCount(
			Wrappers.<User>query().lambda().eq(User::getTenantId, user.getTenantId())
	...
	public boolean updatePassword(Long userId, String oldPassword, String newPassword,
		...
		return this.update(
			Wrappers.<User>update().lambda().set(User::getPassword, DigestUtil.encrypt(newPassword))
	...
	public void importUser(List<UserExcel> data) {
		data.forEach(userExcel -> {
			...
			// 设置岗位ID
			user.setPostId(
				postService.getPostIds(userExcel.getTenantId(), userExcel.getPostName()));
			// 设置角色ID
			user.setRoleId(
				roleService.getRoleIds(userExcel.getTenantId(), userExcel.getRoleName()));
			// 设置默认密码
	...
	public boolean registerGuest(User user, Long oauthId) {
		Tenant tenant = tenantService.getOne(
			Wrappers.<Tenant>lambdaQuery().eq(Tenant::getTenantId, user.getTenantId()));
